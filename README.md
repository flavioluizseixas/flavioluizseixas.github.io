...
Para controlar a sequência das oportunidades abertas, use o campo opcional `ordem` no front matter dos Markdown (por exemplo, `ordem: 1`). Inteiros positivos menores aparecem primeiro; projetos sem esse campo ficam depois dos que têm ordem definida. Empates e projetos sem ordem são classificados pela data de publicação mais recente e, depois, pelo título em ordem alfabética. Esse campo só afeta projetos com `status: inscricoes-abertas`. Veja um exemplo no [guia de conteúdo](data/README.md#projetos-de-extensão).
